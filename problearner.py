...
class PMLearner():
    ...
    def train(self):
        ...
        # feature_state = self.poly.fit_transform(state)
        feature_state = state
    ...
    def model_prediction(self, state, action, mediator):
        ...
        # state = self.rbf_feature.fit_transform(state)
...
class PALearner():
    ...
    def model_prediction(self, state, action):
        ...
        x = self.rbf_feature.fit_transform(state)
        y_prob = self.model.predict_proba(x)
        if state.ndim == 1:
...
#         pa_ratio = []
#         for i, pa_pred1 in enumerate(pa_prediction1):
#             pa_ratio.append(pa_pred1 / pa_prediction2[i])
#         pa_ratio = np.array(pa_ratio)
        return pa_ratio
